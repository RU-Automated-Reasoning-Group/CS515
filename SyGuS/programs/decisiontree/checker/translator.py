...
def ReadQuery(bmExpr):
    ...
    class Checker:
        ...
        def check(self, funcDefStr, moreconstraint):
            self.solver.push()

            spec_smt2 = [funcDefStr]
            # for constraint in Constraints:
            #     spec_smt2.append('(assert %s)' % (toString(constraint[1:])))
            if len(moreconstraint) == 0:
                for constraint in Constraints:
                    spec_smt2.append('(assert %s)' % (toString(constraint[1:])))
            else:
                for constraint in Constraints:
                    r = '(assert '
                    for m in moreconstraint:
                        r += '(=> '
                        r += toString((m[1:]))
                        r += ' '
                    r += toString(constraint[1:])
                    for m in moreconstraint:
                        r += ')'
                    r += ')'
                    spec_smt2.append(r)
                    #spec_smt2.append('(assert (=> %s %s))' % (toString(moreconstraint[0][1:]), toString(constraint[1:])))
            # for constraint in moreconstraint:
            #     spec_smt2.append('(assert %s)' % (toString(constraint[1:])))
            spec_smt2 = '\n'.join(spec_smt2)
            # print spec_smt2
